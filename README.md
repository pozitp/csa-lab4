...
Архитектура Гарвардская: память команд и память данных разделены.

Бинарный файл содержит и код, и данные как загрузочный образ. После загрузки модель кладёт инструкции в `code memory`, а данные в `data memory`. Команды записи не могут изменить `code memory`.

Машинное слово: 32 бита, знаковая арифметика в дополнительном коде.
...
Вход в прерывание добавляет отдельный tick `irq` перед `fetch` обработчика. Память данных считается однотактной; `mul/div/mod` имеют дополнительный tick `alu`; `vld/vst` считают обращение к памяти по каждому из 4 lane.
...
| --- | --- |
| `asm` | язык в `programs/*.asm`, транслятор в `translator.py` |
| `acc` | `ACC`, аккумуляторные scalar-инструкции |
| `harv` | отдельные `code` и `data` memory |
| `hw` | hardwired dispatch в `Machine.execute()` |
| `tick` | trace по тактам: `fetch`, `addr`, `mem`, `exec`, `alu`, `vexec`, `irq` |
| `binary` | бинарный `.bin` с magic `CSA4` |
| `trap` | input schedule + interrupt handler на ассемблере |
| `mem` | `%in`, `%out`, `%ready` отображены в память |
| `pstr` | `.pstr`, строки в памяти данных |
| `prob1` | largest palindrome product, вывод `906609` |
| `vector` | `v0..v15`, vector ALU, сравнение с scalar baseline |

## Вывод

Работа соответствует варианту `asm | acc | harv | hw | tick | binary | trap | mem | pstr | prob1 | vector`. В репозитории есть транслятор, модель процессора, программы на assembly, golden tests, схемы процессора и CI.
